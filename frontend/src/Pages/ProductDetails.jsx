...
const ProductDetails = () => {
  ...
  useEffect(() => {
    if (product?.variants?.length) {
      const fallbackVariant = product.variants.find((variant) => Number(variant.stock_quantity || 0) > 0) || product.variants[0];
            ...
            <div className="details-section">
              ...
              {product.variants?.length > 0 && (
                <div className="variant-selector">
                  <label className="variant-label">Choose size/weight</label>
                  <div className="variant-options">
                    {product.variants.map((variant) => (
                      ...
                      >
                        {variant.variant_name || `${variant.quantity || ""} ${variant.unit || ""}`.trim()}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="action-row">
                <div className={`qty-selector ${currentStock === 0 ? "disabled" : ""}`}>
                  <button disabled={currentStock === 0} onClick={() => setQuantity((q) => Math.max(1, q - 1))}><i className="bi bi-dash"></i></button>
                  <span>{currentStock === 0 ? 0 : quantity}</span>
                  <button disabled={currentStock === 0} onClick={() => setQuantity((q) => q + 1)}><i className="bi bi-plus"></i></button>
                </div>
              </div>
